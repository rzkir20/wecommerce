...
export function SellerHeader({ userName, userEmail }: SellerHeaderProps) {
  const displayName = userName?.trim() || 'Seller'
  const email = userEmail?.trim() || 'seller@wecommerce.app'
    ...
    <header className="sticky top-0 z-40 h-20 border-b border-border bg-background/95 backdrop-blur">
      <div className="mx-auto flex h-full w-full max-w-[1600px] items-center justify-between gap-4 px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden items-center gap-2 rounded-xl bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-700 md:flex dark:text-emerald-300">
